...
**/
trigger EmailAlertField on OpportunityLineItem (before insert, before update,after insert, after update) {
		...
		*/
    	...
    	for(OpportunityLineItem OLI:system.trigger.new)
        {
        	if(OLI.Product2ID!=null  && OLI.revenue_type__c!=null && OLI.revenue_type__c!='')
        	{
				...
				String ProductCode;
				String SAPCode;
				if(O.RecordType.Name.Contains('Non GSM')||O.RecordType.Name.Contains('IOT')||O.RecordType.Name.Contains('Marketplace') )
				{
	        		if(OLI.Revenue_Type__c=='Recurring')
        			{
						...
						if(O.Service_type__c=='E-Commerce'||test.isrunningtest())
						{
