...
**/
trigger trigger_circuit_v2 on Circuit__c (after insert, after update, before insert, before update) {
    ...
    system.debug ('== beforeInsert : ' + isActiveTriggerMap.get ('CircuitTrigger.beforeInsert'));
    if( triggerControllerList.size() > 0 ){
        if ( isActiveTriggerMap.get ('CircuitTrigger') <> null ) {
            if ( isActiveTriggerMap.get ('CircuitTrigger') ) {
                triggerCircuitHandler handler = new triggerCircuitHandler(Trigger.isExecuting, Trigger.size);
                
                if(Trigger.isInsert && Trigger.isBefore && isActiveTriggerMap.get ('CircuitTrigger.beforeInsert') ) {
                    System.debug('Circuit Before Insert Running');
    ...
    //                  string nameCircuit= handler.OnBeforeInsert(C.Services__c);
    //                  C.Name=nameCircuit;


    //                 //     ServiceSetting__c serviceSetting = ServiceSetting__c.getValues(C.Services__c); 
    //                 //     if(C.Services__c == serviceSetting.name){
    //                 //         CIDInitial=serviceSetting.CircuitPrefixName__c;
    //                 //         system.debug('CIDInitial-->'+CIDInitial);
    //                 //         Decimal dlastCounter = serviceSetting.CircuitLastCounterNumber__c;
    //                 //         Integer intlastCounter = dlastCounter.intValue();
    //                 //         CIDNumber=String.valueOf(intlastCounter).leftPad(5, '0');
    //                 //         serviceSetting.CircuitLastCounterNumber__c = dlastCounter+1;
    //                 //         update serviceSetting;
    //                 //     }
    //                 //  String circuitName = CIDInitial+'-'+CIDNumber;
    //                 // C.Name=circuitName;
