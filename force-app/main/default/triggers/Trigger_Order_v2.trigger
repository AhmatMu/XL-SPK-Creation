/**
 * @description       : 
 * @Test Class        : Trigger_Order_v2_Test
...
trigger Trigger_Order_v2 on Order (after insert, after update, before delete, before insert, before update) {
    if (label.Is_Trigger_ORDER_v2_ON == 'YES') {
        string errMessage='';
        Map<ID, User> userMap=new Map<ID,User>([SELECT id,Managerid,Manager.Managerid, email FROM User ]);
        Set<String> set_OrderSodoId = new Set<String>();
        ...
        if(trigger.isupdate){
            ...
            if(trigger.isbefore) {
                for(Order newOrder:system.trigger.new) {
                    ...
                    if(
                        newOrder.recordtypeid == system.label.RT_ORDER_GSM_SIM_Card &&
                        OldOrder.Status != NewOrder.Status && 
                        NewOrder.Status == 'Submit Order' &&
                        newOrder.Approval_Status__c != 'Approved' 
                    ){
                        NewOrder.isReadyToApproval__c = true;
                    }
                }
            }
            ...
            if(trigger.isAfter) {
                ...
                for(Order newOrder:system.trigger.new) {
                    ...
                    */
                    ...
                    if (newOrder.recordtypeid==system.label.RT_ORDER_GSM_SIM_Card) {
                        ...
                        /* --- Di tutup untuk new version SODO process by novando 06/12/2022 (project bizcare)
                        ...
                        if ( oldOrder.status <> 'Order Fulfillment' && newOrder.status == 'Order Fulfillment' 
                            ...
                            set_OrderSodoId.add(newOrder.id);
                        }
                ...
                // add by vando 09-01-2023
                // call API Check Price SAP dan Create Order Sodo Easy Ops
                system.debug('===== set_OrderSodoId : ' + set_OrderSodoId);
                if(set_OrderSodoId!=null && !set_OrderSodoId.isEmpty()){
                    list<OrderItem> List_OrderProduct = [
                        SELECT Id,
                        OrderId,
                        Product_Id__c
                        FROM OrderItem
                        WHERE OrderId In: set_OrderSodoId
                    ];

                    system.debug('===== List_OrderProduct : ' + List_OrderProduct);

                    if(List_OrderProduct!=null && !List_OrderProduct.isEmpty()){
                        Set<String> Set_OrderProductId = new Set<String>();
                        Set<String> Set_ProductId = new Set<String>();
            
                        for(OrderItem orderProductREC : List_OrderProduct) {
                            Set_OrderProductId.add(orderProductREC.Id);
                            Set_ProductId.add(orderProductREC.Product_Id__c);
                        }
