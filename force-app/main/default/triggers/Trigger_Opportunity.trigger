/* ====================================================================================================
* Class Name		        : Trigger_Opportunity
* Test Class		        : TEST_Trigger_Opportunity3
* Created By		        : 
* Created Date		        : 
* Created Description		: - 
* 					          - 
...
* Updated Description		: - ketika opp stage close-lose, record profitability dihapus (After Update)
* 					          - ketika opp Amount/Onetime_revenue ter-update, update value deal price/ Onetime_revenue profitability (After Update)
* ====================================================================================================
*/
trigger Trigger_Opportunity on Opportunity(after insert, after update, before insert, before update) {
    if (system.label.is_Trigger_Opportunity_On == 'YES') {
        ...
        if (trigger.isupdate) {
            if (trigger.isbefore) {
                ...
                for (Opportunity O: system.trigger.new) {
                    ...
                    if (O.RecordtypeName__c == 'GSM (Activation)' && O.StageName == 'Prospecting' && O.StageName != Old.StageName && Old.StageName == 'Submit PO') {
                    ...
                    if (Old.StageName != 'Prospecting' &&
                        ...
                        (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT'))) // && O.Createddate>datelimit)
                    {
                    ...
                    if (Old.StageName != 'Survey' && O.StageName == 'Survey' && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT'))) {
                        O.Survey_Request_Date_Email__c = String.valueof(system.today());
                        
                    }
                    if (Old.StageName != 'Survey' && O.StageName == 'Survey' && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT')) && O.Expected_RFS_Date__c == null && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT'))) //&& O.Link_Related__c!=null)
                    {
                        O.adderror('RFS Date cannot be null');
                    }
                    if (Old.StageName != 'Survey' && O.StageName == 'Survey' && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT')) && O.RecordtypeName__c != 'M-Ads' && O.Service_Type__c == null && O.RecordtypeName__c.Contains('Non GSM')) //&& O.Link_Related__c!=null)
                    {
...
O.Auto_Renewal_UOM__c=SplitAR[1];
                    ...
                    if (Old.StageName != 'Survey' && O.StageName == 'Survey' && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT')) && O.RecordtypeName__c != 'M-Ads') //&& O.Link_Related__c!=null)
                    {
                        if (O.RecordtypeName__c.Contains('Leased Line')) {
                            ...
                            if (ACRSurvey.size() < 2 && !test.isrunningtest()) {
                                ...
                                if ((AccountSiteA == '' || AccountSiteB == '') && !test.isrunningtest()) {
                                    ...
                                    list < SR_PR_Notification__c > SRCek = [
                                        SELECT id FROM SR_PR_Notification__c WHERE 
                                        Opportunity__c =: O.id AND(
                                            (
                                                Status__c = 'Close(Complete)' AND 
                                                Reasons__c <> 'R28-NO PR Submission'
                                            ) OR 
                                            Status__c = 'In Progress' OR 
                                            Status__c = 'Assigned'
                                        )
                                    ];

                                    system.debug('SRCek 259 : ' + SRCek);
                    ...
                    if ((Old.StageName != 'Survey' && O.StageName == 'Survey' && O.RecordtypeName__c == 'B2B Marketplace') || test.isRunningTest()) {
...
CPS.CreateSRComplete(O); 
                    ...
                    if (Old.StageName != 'Quotation FInal' && O.StageName == 'Quotation Final' && (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT'))) // && !test.isrunningtest())//&& O.Link_Related__c!=null)
                    {
                        String RoleBAPrint = '';
                        ...
                        /*revised by Surya 31 Jan 2020 validation quotation final need PIC BA Recipient BA Print etc*/
                        ...
                        for (AccountContactRelation ACR: listACR) {
                            ...
                            if (Acr.Accountid == O.Account_Site_A__c && ACR.Roles != null) {
                                ...
                                /*  
string[] ss = ACR.Roles.split(';');
for (string s:ss) {
//system.debug('======== value : ' + s);
if (s == 'PIC Site') PICSite='TRUE'   ;
}
*/
                        ...
                        //  list<AccountContactRelation> listPrint=[SELECT id,Roles,AccountID,Contact.Name,Account.BP_Number__c,Contactid,Contact.Email FROM AccountContactRelation WHERE AccountID=:O.Accountid AND Roles INCLUDES('PIC BA Print') AND Contactid=:O.PIC_BA_Print__c];
                        ...
                        if (PICSite == '' && !test.isrunningtest())
                            O.AddError('No PIC Site in Site A');
...
*/
                    ...
                    if (Old.StageName != 'Waiting for Contract' /* && Old.StageName!='Waiting for BA'*/ && ( /*O.StageName=='Waiting for BA'||*/ O.StageName == 'Waiting for Contract') && ((O.RecordtypeName__c.Contains('Non GSM') && O.OneTime_Revenue__c == 0 && O.Recurring_Revenue__c == 0) || O.RecordtypeName__c.Contains('VoIP'))) {
                        O.StageName = 'Closed Won';
                    }
                    if (O.StageName == 'Waiting for BA' && Old.StageName == 'Implementation' && O.Contract_Ticket__c == null && O.Lastmodifieddate < system.now().addseconds(-1) && !test.isrunningtest() && O.RecordtypeName__c == 'B2B Marketplace') {
                    ...
                    if (
                        ...
                        !test.isrunningtest() && 
                        (O.RecordtypeName__c.Contains('Non GSM') || O.RecordtypeName__c.Contains('IoT'))
                    ) {
                    ...
                    if (Old.StageName != 'Closed Won' && O.StageName == 'Closed Won' && O.RecordtypeName__c != 'GSM' && O.RecordtypeName__c.Contains('Non GSM')) //&& O.Link_Related__c!=null)
                    {
...
*/
                    if ((O.Service_Type__c == 'Upgrade' || O.Service_Type__c == 'Downgrade' || O.Service_Type__c == 'Reroute') &&
                        Old.Link_Related__c <> O.Link_Related__c && O.Link_Related__c <> null && O.RecordtypeName__c.contains('Non GSM') && !O.RecordtypeName__c.Contains('VoIP'))
...
*/
                    ...
                    string errMessage = '';
                    if ((O.SERVICE_TYPE__C == 'Newlink' || O.Recordtypename__c == 'Digital Advertising' || O.Recordtypename__c == 'M_Ads') &&
...
O.Periode_UOM__c=C.Periode_UOM__c;*/
            ...
            if (trigger.isafter) {
                for (Opportunity Opp: system.trigger.new) {
                    ...
                    if (Old.StageName != 'Implementation' && Opp.StageName == 'Implementation' && (Opp.RecordtypeName__c.Contains('Non GSM') || Opp.RecordtypeName__c.Contains('IoT')) && Opp.RecordTypeName__c != 'Digital Advertising' && !test.isrunningtest() && (Opp.PR__C == '' || Opp.PR__C == null)) // && O.Createddate>datelimit)//  && O.Link_Related__c!=null)
                    {  
                        //PR created when implementation
                        Create_PR_SR CPS = new Create_PR_SR();
                        CPS.CreatePR(Opp);
                    }
                    if (Old.StageName != 'Implementation' && Opp.StageName == 'Implementation' && Opp.recordtypename__c.contains('Marketplace') && Opp.RecordTypeName__c != 'Digital Advertising' && !test.isrunningtest() && (Opp.PR__C == '' || Opp.PR__C == null)) // && O.Createddate>datelimit)//  && O.Link_Related__c!=null)
                    {
                        //PR created when implementation marketplace
                        Create_PR_SR CPS = new Create_PR_SR();
                    ...
                    if(Old.StageName!='Waiting for Contract' 
                       && Opp.StageName=='Waiting for Contract' 
                       && !Opp.RecordtypeName__c.Contains('VoIP') 
                       && ( Opp.RecordtypeName__c.Contains('Non GSM')
                           || Opp.RecordtypeName__c.Contains('IoT')
                           || Opp.recordtypeName__c.contains('Marketplace')
                           || Opp.recordtypeName__c.contains('Digital Advertising'))
                      )
                    {
                        ...
                        if (Opp.Contract_Ticket__c != null) { //-- contrak ticket is available
                            ...
                            Contract_Services contractServices = new Contract_Services();
                            contractServices.updateContractsFromOpportunity(Opp.id);
...
O.AddError('One Of Product\'s SAP Code is Empty, Please fill it before creating contract');
                            ...
                            if(Opp.recordtypeName__c!='B2B Marketplace' && Opp.recordtypeName__c!='Digital Advertising')
                            {
                                ...
                                if (String.valueof(Opp.BP_Payer__c) == '')
                                    Opp.AddError('BP Payer is Empty, Please fill it before creating contract');
                            } else if (Opp.recordtypeName__c!='Digital Advertising')
                            {
                            ...
                            contractServices.CreateContractsFromOpportunity(Opp.id);
                        }
                        ...
                        if (Opp.Service_Type__c == 'Relocation') {
                            if (Opp.Recordtypename__c.Contains('Leased Line')) {
